...
interface Props {
    jadwal?: Record<string, JadwalPelajaran[]>;
    kelas?: Kelas[];
...
const colorSchemes = [
    { bg: 'from-blue-500 to-blue-600', shadow: 'shadow-blue-500/50', border: 'border-blue-400' },
    { bg: 'from-purple-500 to-purple-600', shadow: 'shadow-purple-500/50', border: 'border-purple-400' },
    { bg: 'from-pink-500 to-pink-600', shadow: 'shadow-pink-500/50', border: 'border-pink-400' },
    { bg: 'from-green-500 to-green-600', shadow: 'shadow-green-500/50', border: 'border-green-400' },
    { bg: 'from-yellow-500 to-yellow-600', shadow: 'shadow-yellow-500/50', border: 'border-yellow-400' },
    { bg: 'from-red-500 to-red-600', shadow: 'shadow-red-500/50', border: 'border-red-400' },
    { bg: 'from-indigo-500 to-indigo-600', shadow: 'shadow-indigo-500/50', border: 'border-indigo-400' },
    { bg: 'from-teal-500 to-teal-600', shadow: 'shadow-teal-500/50', border: 'border-teal-400' },
];
...
export default function Index({ jadwal, kelas, selectedKelas }: Props) {
    ...
    const handleDelete = (id: number, hari: string, mapel: string) => {
        if (confirm(`Apakah Anda yakin ingin menghapus jadwal ${mapel} hari ${hari}?`)) {
    ...
    // Safe check for jadwal
    const hasJadwal = jadwal && Object.keys(jadwal).length > 0;
            ...
            <div className="mb-6 flex flex-col sm:flex-row gap-4 items-start sm:items-center justify-between">
                <div className="flex-1 max-w-md">
                    ...
                    >
                        ...
                        {kelas && Array.isArray(kelas) && kelas.map((k) => (
                            <option key={k.id} value={k.id}>
                                {k.jenjang?.nama || 'N/A'} - {k.nama_kelas}
                            </option>
                        ))}
                    </select>
                </div>
                                    ...
                                    <div className="p-4 space-y-3">
                                        {jadwalHari.map((item, index) => {
                                            ...
                                            return (
                                                <div
                                                    key={item.id}
                                                    className={`relative rounded-xl overflow-hidden transform hover:scale-105 transition-all duration-300 ${colors.shadow} shadow-lg hover:shadow-xl`}
                                                >
                                                    {/* Gradient Background */}
                                                    <div className={`bg-gradient-to-br ${colors.bg} p-4 text-white`}>
                                                        {/* Time Badge */}
                                                        ...
                                                        <div className="flex items-center space-x-2 mb-3 bg-white/10 backdrop-blur-sm px-3 py-2 rounded-lg">
                                                            <div className="w-8 h-8 bg-white/30 rounded-full flex items-center justify-center font-bold">
                                                                {item.guru.nama_lengkap.charAt(0)}
                                                            </div>
                                                        ...
                                                        <div className="flex gap-2">
                                                            ...
                                                            <button
                                                                onClick={() => handleDelete(item.id, hari, item.mata_pelajaran.nama)}
