...
interface Tagihan {
    ...
    total: number;
    jatuh_tempo: string;
    status: 'belum_bayar' | 'dibayar_sebagian' | 'lunas';
}
...
export default function Index({ tagihan }: Props) {
    const handleDelete = (id: number, nama: string) => {
        if (confirm(`Apakah Anda yakin ingin menghapus tagihan untuk "${nama}"?`)) {
            router.delete(route('keuangan.tagihan.destroy', id));
        }
    };
    ...
    const getBulanNama = (bulan: number) => {
        ...
        return namaBulan[bulan - 1] || bulan;
    };
    ...
    return (
        <SidebarLayout>
            ...
            <div className="flex justify-between items-center mb-6">
                ...
                <Link
                    href={route('keuangan.tagihan.create')}
                                        ...
                                        {tagihan.data.length === 0 ? (
                                            ...
                                            tagihan.data.map((item, index) => (
                                                <tr key={item.id} className="hover:bg-gray-50">
                                                    ...
                                                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                                        {item.bulan ? `${getBulanNama(item.bulan)} ${item.tahun}` : item.tahun}
                                                    </td>
                                                    ...
                                                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                                        <div className="flex space-x-2">
                                                            <Link
                                                                href={route('keuangan.tagihan.edit', item.id)}
                            ...
                            {tagihan.last_page > 1 && (
                                <div className="mt-6 flex items-center justify-between">
                                    <div className="text-sm text-gray-700">
                                        Menampilkan <span className="font-medium">{(tagihan.current_page - 1) * tagihan.per_page + 1}</span> sampai{' '}
                                        <span className="font-medium">
                                            {Math.min(tagihan.current_page * tagihan.per_page, tagihan.total)}
                                        </span>{' '}
                                        dari <span className="font-medium">{tagihan.total}</span> data
                                    </div>
