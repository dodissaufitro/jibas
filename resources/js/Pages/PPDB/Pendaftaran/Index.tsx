...
export default function Index({ pendaftaran }: Props) {
    const handleDelete = (id: number, nama: string) => {
        if (confirm(`Apakah Anda yakin ingin menghapus pendaftaran "${nama}"?`)) {
            router.delete(route('ppdb.pendaftaran.destroy', id));
        }
    };
    ...
    const getStatusBadge = (status: string) => {
        const badges: Record<string, { bg: string; text: string; label: string }> = {
            ...
            lulus: { bg: 'bg-green-100', text: 'text-green-800', label: 'Lulus' },
            tidak_lulus: { bg: 'bg-red-100', text: 'text-red-800', label: 'Tidak Lulus' },
        };
    ...
    return (
        <SidebarLayout>
            <Head title="Data Pendaftaran PPDB" />
            ...
            <div className="flex justify-between items-center mb-6">
                ...
                <Link
                    href={route('ppdb.pendaftaran.create')}
                                        ...
                                        {pendaftaran.data.length === 0 ? (
                                            ...
                                            pendaftaran.data.map((item, index) => (
                                                <tr key={item.id} className="hover:bg-gray-50">
                                                    ...
                                                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                                        {item.jurusan ? item.jurusan.nama : '-'}
                                                    </td>
                                                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                                    ...
                                                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                                        <div className="flex space-x-2">
                                                            <Link
                                                                href={route('ppdb.pendaftaran.edit', item.id)}
